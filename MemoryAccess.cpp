...
void MemoryAccess::printAlbums() 
{
	...
	for (const Album& album: m_albums) 	{
		std::cout << " Creation Time: " << album.getCreationDate() << std::setw(5) << "* " << album;
	}
}

bool MemoryAccess::isDBType()
{
	return false;
}
...
void MemoryAccess::cleanUserData(const User& userId)
{
	for (auto iter2 = m_albums.begin(); iter2 != m_albums.end(); ++iter2) {
		if (iter2->getOwnerId() == userId.getId()) {
			iter2 = m_albums.erase(iter2);
...
bool MemoryAccess::doesPictureExists(std::string picName)
{
	return false;
}

Picture MemoryAccess::getPicture(std::string picName)
{
	return Picture(-1, "-");
}
...
int MemoryAccess::countTagsOfPicture(int pictureID)
{
	return 0;
}

void MemoryAccess::listPicturesInAlbum(std::string albumName)
{
}
