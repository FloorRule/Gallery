...
void AlbumManager::listPicturesInAlbum()
{
	...
	for (auto iter = albumPictures.begin(); iter != albumPictures.end(); ++iter) {
		std::cout << "   + Picture [" << iter->getId() << "] - " << iter->getName() <<
			...
			iter->getCreationDate() << "]\tTags: [" << m_dataAccess.countTagsOfPicture(iter->getId()) << "]" << std::endl;
...
void signal_callback_handler(int signum) {
	...
	// Terminate program
	if (killProcessByName("i_view64.exe") && index == 1)
	{
		printf("IrfanView-64.exe Terminate successfully!");
	}
	if (killProcessByName("mspaint.exe") && index == 2)
	{
		printf("Mspaint.exe Terminate successfully!");
	}
	//closing the loop so we can continue with the program
	flag = 0;
}
...
void AlbumManager::showPicture()
{
	...
	std::string fullPath;
	//send you to a editor choosing func
	if (choosingPictureEditor())
	{
		//creating th full system path
		fullPath = "mspaint.exe " + pic.getPath();
		index = 1;
	}
	else
	{
		fullPath = "start C:\\Users\\משתמש\\Desktop\\i_view64.exe " + pic.getPath();
		index = 2;
	}
	
	//turning on the loop
	flag = 1;
	//sending the path to a thread so the program could still work
	char* ptrPath = (char*)fullPath.c_str();
	HANDLE WINAPI hThread = CreateThread(NULL, 0, thread_main, ptrPath, 0, NULL);
	//starting a listen for ctrl+c
	signal(SIGINT, signal_callback_handler);
	WaitForSingleObject(hThread, INFINITE);
}

bool choosingPictureEditor()
{
	int option = 0;
	std::cout << "Choose Editor to open picture with:" << std::endl;
	std::cout << "	-[1] Mspaint.exe" << std::endl << "	-[2] IrfanView-64.exe" << std::endl;
	...
	{
		std::cout << "ERROR: enter again numbers 1-2";
		while (!(std::cin >> option))
		{
			// Clear errors (like the failbit flag)
			std::cin.clear();
...
void AlbumManager::tagUserInPicture()
{
	...
	m_openAlbum.tagUserInPicture(user.getId(), pic.getName());
...
void AlbumManager::untagUserInPicture()
{
	...
	m_openAlbum.untagUserInPicture(user.getId(), pic.getName());
...
void AlbumManager::userStatistics()
{
	...
	std::cout << "user @" << userId << " Statistics:" << std::endl << "--------------------" << std::endl <<
		...
		"  + Count of Albums Created by User: " << m_dataAccess.countAlbumsOwnedOfUser(user) << std::endl;
}
...
{
	struct stat buffer; 
	//std::cout << filename.c_str() << " " << stat(filename.c_str(), &buffer) << " " << (stat(filename.c_str(), &buffer) == 0) <<std::endl;
	return (stat(filename.c_str(), &buffer) == 0); 
}

void AlbumManager::refreshOpenAlbum() {
	//std::cout << "mode: " << isCurrentAlbumSet() << std::endl;
	if (!isCurrentAlbumSet()) {
